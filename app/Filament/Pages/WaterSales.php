...
class WaterSales extends Page implements HasTable
{
    use InteractsWithTable;

    protected string $view = 'filament.pages.water-sales';

    protected static string|BackedEnum|null $navigationIcon = Heroicon::OutlinedBeaker;

    protected static ?string $navigationLabel = 'Vendas de água';

    protected static ?string $title = 'Vendas de água';
    ...
    {
        return $table
            ->query(Sale::query()->where('contains_water', true)->with('customer'))
            ...
            ->columns([
                ...
                IconColumn::make('mixed')->label('Mista')
                    ->getStateUsing(fn ($record) => $record->items->some(
                        fn ($item) => optional(optional($item->variant)->product?->category)->slug !== 'agua',
                    ))
                    ->boolean(),
